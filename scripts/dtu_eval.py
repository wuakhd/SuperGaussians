...
if not args.skip_training:
    common_args = " --quiet --depth_ratio 1.0 -r 2 --lambda_dist 1000 --max_gs 100_000"
    # common_args = " -r 2"
    for scene in dtu_scenes:
        source = args.dtu + "/" + scene
        print("python train2.py -s " + source + " -m " + args.output_path + "/" + scene + common_args)
        os.system("python train2.py -s " + source + " -m " + args.output_path + "/" + scene + common_args)

    # for scene in dtu_scenes:
    #     source = args.dtu + "/" + scene
    #     print("python train.py -s " + source + " -m " + args.output_path + "/" + scene )
    #     os.system("python train.py -s " + source + " -m " + args.output_path + "/" + scene)
...
if not args.skip_metrics:
    ...
    for scene in dtu_scenes:
        ...
        string = f"python {script_dir}/eval_dtu/evaluate_single_scene.py " + \
            ...
            f"--scan_id {scan_id} --output_dir {args.output_path}/{scene}/tmp/scan{scan_id} " + \
            f"--mask_dir {args.dtu} " + \
