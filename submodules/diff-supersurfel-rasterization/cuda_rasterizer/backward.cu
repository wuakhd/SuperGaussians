...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs, float2* wichzone)
{
	...
	glm::vec3* dL_dsh = dL_dshs + idx * (24);
	...
	float* __restrict__ dL_dshs)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			glm::vec3 dL_daddrgb[4];
			dL_daddrgb[0] = glm::vec3(0,0,0);
			dL_daddrgb[1] = glm::vec3(0,0,0);
			dL_daddrgb[2] = glm::vec3(0,0,0);
			dL_daddrgb[3] = glm::vec3(0,0,0);
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				const float c = collected_colors[ch * BLOCK_SIZE + j] +  nncolor[ch];
				...
				dL_daddrgb[0][ch] =  my_weight.x * dl_dc[ch] / 4.0f;
				dL_daddrgb[1][ch] =  my_weight.y * dl_dc[ch] / 4.0f;
				dL_daddrgb[2][ch] =  my_weight.z * dl_dc[ch] / 4.0f;
				dL_daddrgb[3][ch] =  my_weight.w * dl_dc[ch] / 4.0f;
			}
			
			
			atomicAdd(&dL_dshs[72*global_id + 48 ], dL_daddrgb[0].x);
			atomicAdd(&dL_dshs[72*global_id + 49 ], dL_daddrgb[0].y);
			atomicAdd(&dL_dshs[72*global_id + 50 ], dL_daddrgb[0].z);
			atomicAdd(&dL_dshs[72*global_id + 51 ], dL_daddrgb[1].x);
			atomicAdd(&dL_dshs[72*global_id + 52 ], dL_daddrgb[1].y);
			atomicAdd(&dL_dshs[72*global_id + 53 ], dL_daddrgb[1].z);
			atomicAdd(&dL_dshs[72*global_id + 54 ], dL_daddrgb[2].x);
			atomicAdd(&dL_dshs[72*global_id + 55 ], dL_daddrgb[2].y);
			atomicAdd(&dL_dshs[72*global_id + 56 ], dL_daddrgb[2].z);
			atomicAdd(&dL_dshs[72*global_id + 57 ], dL_daddrgb[3].x);
			atomicAdd(&dL_dshs[72*global_id + 58 ], dL_daddrgb[3].y);
			atomicAdd(&dL_dshs[72*global_id + 59 ], dL_daddrgb[3].z);

			atomicAdd(&dL_dshs[72*global_id + 60 ], (dl_dc[0] * -addrgb[0].x + dl_dc[1] * -addrgb[0].y + dl_dc[2] * -addrgb[0].z + my_o.x*0.0) *exp_scale[0]*exp(-exp_scale[0]*((kernal[0].x - proj_u)*(kernal[0].x - proj_u) + (kernal[0].y - proj_v)*(kernal[0].y - proj_v)))*(2*kernal[0].x - 2*proj_u) / 4.0f );
			atomicAdd(&dL_dshs[72*global_id + 61 ], (dl_dc[0] * -addrgb[0].x + dl_dc[1] * -addrgb[0].y + dl_dc[2] * -addrgb[0].z + my_o.x*0.0) *exp_scale[0]*exp(-exp_scale[0]*((kernal[0].x - proj_u)*(kernal[0].x - proj_u) + (kernal[0].y - proj_v)*(kernal[0].y - proj_v)))*(2*kernal[0].y - 2*proj_v) / 4.0f );
			
			atomicAdd(&dL_dshs[72*global_id + 63 ], (dl_dc[0] * -addrgb[1].x + dl_dc[1] * -addrgb[1].y + dl_dc[2] * -addrgb[1].z + my_o.y*0.0) *exp_scale[1]*exp(-exp_scale[1]*((kernal[1].x - proj_u)*(kernal[1].x - proj_u) + (kernal[1].y - proj_v)*(kernal[1].y - proj_v)))*(2*kernal[1].x - 2*proj_u) / 4.0f );
			atomicAdd(&dL_dshs[72*global_id + 64 ], (dl_dc[0] * -addrgb[1].x + dl_dc[1] * -addrgb[1].y + dl_dc[2] * -addrgb[1].z + my_o.y*0.0) *exp_scale[1]*exp(-exp_scale[1]*((kernal[1].x - proj_u)*(kernal[1].x - proj_u) + (kernal[1].y - proj_v)*(kernal[1].y - proj_v)))*(2*kernal[1].y - 2*proj_v) / 4.0f );
			
			atomicAdd(&dL_dshs[72*global_id + 66 ], (dl_dc[0] * -addrgb[2].x + dl_dc[1] * -addrgb[2].y + dl_dc[2] * -addrgb[2].z + my_o.z*0.0) *exp_scale[2]*exp(-exp_scale[2]*((kernal[2].x - proj_u)*(kernal[2].x - proj_u) + (kernal[2].y - proj_v)*(kernal[2].y - proj_v)))*(2*kernal[2].x - 2*proj_u) / 4.0f );
			atomicAdd(&dL_dshs[72*global_id + 67 ], (dl_dc[0] * -addrgb[2].x + dl_dc[1] * -addrgb[2].y + dl_dc[2] * -addrgb[2].z + my_o.z*0.0) *exp_scale[2]*exp(-exp_scale[2]*((kernal[2].x - proj_u)*(kernal[2].x - proj_u) + (kernal[2].y - proj_v)*(kernal[2].y - proj_v)))*(2*kernal[2].y - 2*proj_v) / 4.0f );
			
			atomicAdd(&dL_dshs[72*global_id + 69 ], (dl_dc[0] * -addrgb[3].x + dl_dc[1] * -addrgb[3].y + dl_dc[2] * -addrgb[3].z + my_o.w*0.0) *exp_scale[3]*exp(-exp_scale[3]*((kernal[3].x - proj_u)*(kernal[3].x - proj_u) + (kernal[3].y - proj_v)*(kernal[3].y - proj_v)))*(2*kernal[3].x - 2*proj_u) / 4.0f );
			atomicAdd(&dL_dshs[72*global_id + 70 ], (dl_dc[0] * -addrgb[3].x + dl_dc[1] * -addrgb[3].y + dl_dc[2] * -addrgb[3].z + my_o.w*0.0) *exp_scale[3]*exp(-exp_scale[3]*((kernal[3].x - proj_u)*(kernal[3].x - proj_u) + (kernal[3].y - proj_v)*(kernal[3].y - proj_v)))*(2*kernal[3].y - 2*proj_v) / 4.0f );
			...
			atomicAdd(&(dL_dopacity[global_id*4 +0]), G * dL_dalpha *  my_weight.x / 4.0f);
			atomicAdd(&(dL_dopacity[global_id*4 +1]), G * dL_dalpha *  my_weight.y / 4.0f);
			atomicAdd(&(dL_dopacity[global_id*4 +2]), G * dL_dalpha *  my_weight.z / 4.0f);
			atomicAdd(&(dL_dopacity[global_id*4 +3]), G * dL_dalpha *  my_weight.w / 4.0f);
